...
std::vector<int> Arena::GetDuelResult() {
    ...
    for (int i = 0; i < 3; ++i) {
        auto curA = lineup_a_[i];
        auto curB = lineup_b_[i];
        int countA = 0, countB = 0;
        for (auto tmp = curA; tmp; tmp = tmp->next) {
            ++countA;
        }
        for (auto tmp = curB; tmp; tmp = tmp->next) {
            ++countB;
        }
        ...
        int survivorsA = countA - removedA;
        int survivorsB = countB - removedB;
        results[i] = survivorsA - survivorsB;
    }
    return results;
}
...
public:
    PartsOrder(Inventory* inventory, const Part& part);
...
public:
    SalesOrder(Inventory* inventory, const Item& item);
    void Process() override;
private:
    Item item_;
};
...
PartsOrder::PartsOrder(Inventory* inventory, const Part& part)
    : Order(inventory), part_(part) {}

void PartsOrder::Process() {
    inventory_->AddPart(part_);
}

SalesOrder::SalesOrder(Inventory* inventory, const Item& item)
    : Order(inventory), item_(item) {}

void SalesOrder::Process() {
    inventory_->SellItem(item_.name, item_.quantity);
}
